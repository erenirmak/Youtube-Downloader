...
class GetProgressive:
    ...
    def GP_Download(self):
        streaming.download(output_path = 'C://YT_Downloads', filename = self)
...
class GetAdaptive:
    ...
    def VideoAdaptive(self, vidn):
        streaming.download(output_path = 'C://YT_Downloads/', filename = self.vidn)
    
    def AudioAdaptive(self, audn):
        audstream = yt.streams.get_audio_only() #find audio with the highest bitrate
        audstream.download(output_path = 'C://YT_Downloads/', filename = self.audn) #download audio with highest bitrate onlypass
        return audstream